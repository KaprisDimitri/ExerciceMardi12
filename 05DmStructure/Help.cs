...
// Fonction pour verifié si c'est un byte avec un max et min
    ...
    public static byte MaxMinByte(string num, byte min, byte max)
    {
        byte value = 0;
       // bool isvalid = true;
       
       
            value = GoodInputByteKey(num);
            if(value== 250 || value > max || value< min)
            {
                value = 250;
                MessageMaxMin(min, max);
            }
    ...
    public static byte GoodInputByteKey (string key)
     {
         if(byte.TryParse(key, out byte num))
         {
                 return num;
         }
         else
